...
#     counters = read_counters('/app/data/counters.json')
#     return counters

def initialize_counters(filepath='/app/data/counters.json'):
    counters = read_counters(filepath)
    return counters
...
def make_prediction(input_df, model, stats, counters, role):
    ...
    input_df_filled, filled_column, current_value = filler(input_df_modified, role, stats)
...
# counters = initialize_counters('src/backend/data/counters.json')
